...
object RandomPositionGenerator {
    ...
    fun generate(nodeEntity: NodeEntity, rX: Int, yY: Int, start: Vector?, onWater: Boolean, aboveLand: Boolean, requireWalkable: Boolean): Vector? {
        ...
        val hasRestriction: Boolean = if (nodeEntity.hasRestriction) {
            nodeEntity.restrictCenter.closerThan(nodeEntity.location.toPosition(), (nodeEntity.restrictRadius + rX.toFloat()).toDouble() + 1.0)
        } else {
            false
        }
        var find = false
        ...
        repeat(10) {
            ...
            var result = Position((x + nodeEntity.x).toInt(), (y + nodeEntity.y).toInt(), (z + nodeEntity.z).toInt())
            if (result.y < 0) {
                return@repeat
            }
     ...
     * 通过条件不断向上移动坐标指针
     */
    fun moveUp(position: Position, y: Int, maxY: Int, check: (Position) -> Boolean): Position {
        return if (y < 0) {
    ...
    private fun randomDelta(random: Random, restrictX: Int, restrictY: Int, vector: Vector?): Position? {
        return if (vector != null) {
            ...
            if (abs(rX) <= restrictX.toDouble() && abs(aX) <= restrictX.toDouble()) {
                Position(rX.toInt(), random.nextInt(2 * restrictY + 1) - restrictY, aX.toInt())
            } else {
                null
            }
        } else {
