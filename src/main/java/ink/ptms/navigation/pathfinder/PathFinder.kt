...
class PathFinder(val nodeReader: NodeReader) {
    ...
    fun findPath(position: Location, distance: Float, distanceManhattan: Int = 1, deep: Float = 1f): Path? {
        return findPath(setOf(Position.at(position)), distance, distanceManhattan, deep)
    }

    fun findPath(position: Position, distance: Float, distanceManhattan: Int = 1, deep: Float = 1f): Path? {
        return findPath(setOf(position), distance, distanceManhattan, deep)
    }

    fun findPath(position: Set<Position>, distance: Float, distanceManhattan: Int = 1, deep: Float = 1f): Path? {
        openSet.clear()
        val start = nodeReader.getStart()
        val map = position.map {
            nodeReader.getGoal(it.x.toDouble(), it.y.toDouble(), it.z.toDouble()) to it
        }
    ...
    private fun findPath(begin: Node, list: List<Pair<NodeTarget, Position>>, distance: Float, distanceManhattan: Int, deep: Float): Path? {
        begin.g = 0.0f
        ...
        var heap = 0
        val set = Lists.newArrayListWithExpectedSize<Pair<NodeTarget, Position>>(list.size)
    ...
    private fun getBestHeuristic(node: Node, list: List<Pair<NodeTarget, Position>>): Float {
    ...
    private fun reconstructPath(node: Node, position: Position, flag: Boolean): Path {
