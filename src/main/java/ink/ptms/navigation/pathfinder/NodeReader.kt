...
open class NodeReader(val entity: NodeEntity) {
    ...
    fun getCachedBlockType(position: Position): PathType {
    ...
    fun getStart(): Node {
        ...
        if (!entity.canStandOnFluid(block.getFluid())) {
            if (entity.canFloat && entity.isInWater()) {
                ...
                blockposition = Position.at(entity.location)
        ...
        blockposition = Position.at(entity.location)
        val blockPathType = getCachedBlockType(blockposition.x, y, blockposition.z)
