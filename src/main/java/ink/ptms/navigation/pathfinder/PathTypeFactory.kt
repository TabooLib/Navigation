...
open class PathTypeFactory(val entity: NodeEntity) {
     ...
     * 那么该方块也会被视为危险类型
     */
    open fun getTypeAsWalkable(world: World, position: Position): PathType {
        // 获取原始类型
     ...
     * 主要目的是获取其临近的危险方块
     */
    open fun getTypeAsNeighbor(world: World, position: Position, pathType: PathType): PathType {
        (-1..1).forEach { ox ->
            (-1..1).forEach { oy ->
                (-1..1).forEach { oz ->
                    if (ox != 0 || oz != 0) {
     ...
     * 不对其临近方块进行判断
     */
    open fun getRawType(world: World, position: Position): PathType {
